...
class Assets {
	...
	public function get_asset_meta( $file, $deps = [], $ver = false ) {
		$asset_meta_file = sprintf( '%s/js/%s.asset.php', untrailingslashit( DEV_TALKS_BUILD_DIR ), basename( $file, '.' . pathinfo( $file )['extension'] ) );
	...
	public function register_script( $handle, $file, $deps = [], $ver = false, $in_footer = true ) {
		...
		$file_path = sprintf( '%s/%s', DEV_TALKS_BUILD_DIR, $file );

		if ( ! \file_exists( $file_path ) ) {
			return false;
		}

		$src        = sprintf( DEV_TALKS_BUILD_URI . '/%s', $file );
